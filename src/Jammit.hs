...
instance PL.PropertyListItem Beat where

  toPropertyList b = PL.plDict $ Map.fromList $ catMaybes
    [ guard (isDownbeat  b) >> Just ("isDownbeat" , PL.plBool $ isDownbeat  b)
    , guard (isGhostBeat b) >> Just ("isGhostBeat", PL.plBool $ isGhostBeat b)
    ,                          Just ("position"   , PL.plReal $ position    b)
    ]
...
loadBeats dir = PL.listFromPropertyList <$>
  readXmlPropertyListFromFile' (dir </> "beats.plist")

loadGhost :: FilePath -> IO (Maybe [Beat])
loadGhost dir = PL.listFromPropertyList <$>
  readXmlPropertyListFromFile' (dir </> "ghost.plist")
...
instance PL.PropertyListItem Section where

  toPropertyList b = PL.plDict $ Map.fromList
    [ ("beat"  , PL.plInt $ sectionBeat   b)
    , ("number", PL.plInt $ sectionNumber b)
    , ("type"  , PL.plInt $ sectionType   b)
    ]
